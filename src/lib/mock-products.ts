...
/**
 * Placeholder catalog used only when the Shopify credentials aren't set, so a
 * fresh clone can run `next dev` and `next build` without a store. Once
 * `SHOPIFY_STORE_DOMAIN` and `SHOPIFY_STOREFRONT_ACCESS_TOKEN` are present,
 * `@/lib/products` reads from Shopify instead and none of this is used.
 *
 * Each `file` names an image under `public/images/products/placeholder/`,
 * shared across every product — these are throwaway placeholders, so there's
 * no per-product folder. See that folder's README for naming.
 */
